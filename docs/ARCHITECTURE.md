...
(`$lib/session/workout-age.ts`). That is what lets the device decide before any request
reaches the server — which matters, because offline is where this decision most often
has to be made.

The window is enforced on the client only, and that is a deliberate scope rather than an
omission. `isResumable` has no server call site: `openWorkoutsForPlan` returns every open
workout whatever its age and lets Home decide what to offer, and `?/start` will hydrate a
workout of any age. Nothing is riding on the server refusing, because the window protects
a *number in an export* rather than access to anything — a client that ignored it would
only corrupt its own session duration, which is the same thing a device with a badly wrong
clock can already do (§14). What the server does check is that the workout belongs to the
route asking for it, since `?resume=` is user-supplied and pairs nothing by itself.
...
| Discarding a workout that a second context still holds retries forever | Accepted, and requires overlap to trigger: a second browser tab on the same device, or a second device, still holds `localStorage`'s resume pointer for a workout, or has sync ops queued against it, when a discard elsewhere deletes that workout server-side. If that other context then writes a `set`/`metric`/`deviation`/`finish` op against it, `replay.ts`'s `requireWorkout` finds nothing and throws `NotYetError` — the identical signal it throws when a workout's `start` simply hasn't synced yet, so there is no way to tell "not yet" from "gone for good." Ordinary workout-scoped ops are supposed to wait out `NotYetError` for exactly that first case, so the op retries indefinitely and the sync banner stays permanently pending with no way for the user to clear it. This is not the `discard` op's own asymmetry — `discard` already treats a missing workout as success — it is every *other* op kind still treating "gone" as "not yet." Closing it means giving replay a way to tell a genuinely discarded workout from one whose `start` is merely late, most plausibly a tombstone row, which is more machinery than a case this narrow has earned. Reopens if a discard is ever seen racing continued use of the same workout from a second context in practice. Note the one direction of this overlap that is *not* accepted and is closed in code: a stale discard replaying against a workout that same overlap has since **finished** would permanently delete a completed session, so `discardWorkout` resolves its target with `completed_at IS NULL` and refuses (CLAUDE.md, Invariants). A stuck pending op is a nuisance; silently deleting logged training data is not |
