...
| A chart's own plotted marks can render under the 44px touch-target floor | Accepted and parked, not fixed. `Sparkline`/`BarChart` size each point's tap target (a hit circle or bar rect) in the chart's own SVG `viewBox` coordinate units; once a chart actually has data to plot, those marks measure under 44px on a narrow phone. Predates the design-system pass and is a chart-primitive bug, not a screen-level layout call. `e2e/touch-targets.spec.ts` only catches it when a route's fixture seeds real duration data — the isolated progress fixtures render the charts' own empty state instead, so a solo run of that spec misses it; the full suite, with `progress-walkthrough.spec.ts` seeding data first, reproduces it. Fixing it needs interaction-design judgement (enlarging the hit circles risks overlapping neighbours on a densely-plotted chart), not a token swap, so it is deferred until it gets its own task |
