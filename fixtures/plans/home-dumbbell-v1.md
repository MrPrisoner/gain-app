...
metrics:
  ...
  session:
    ...
    - key: squash_since_last
      ...
      prompt_when: start
    - key: symptoms_during
      label: Lower-back symptoms during session
      type: scale
      min: 0
      max: 10
      prompt_when: end
    - key: symptoms_after
      label: Lower-back symptoms immediately after
      type: scale
      min: 0
      max: 10
      prompt_when: end
    ...
    - key: symptoms_next_morning
      label: Lower-back symptoms next morning
      type: scale
      min: 0
      max: 10
...
1. **Warm-up applied to all four sessions.** The prose specifies warm-up exercises only
   under Session A, but §7 lists a 5–6 minute warm-up as a component of every session.
   The Session A warm-up has therefore been attached to B, C and D as well.
2. **A `moderate` load configuration was added** at 5 kg per dumbbell. The prose defines
   only heavy (~6 kg), light (~4 kg) and bodyweight, but Session D prescribes "moderate"
